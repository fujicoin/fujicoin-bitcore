...
CPubKey CWallet::GenerateNewKey()
{
    ...
    if (!hdChain.masterKeyID.IsNull()) {
        ...
        CExtKey externalChainChildKey; //key at m/75'/0'
...
bool CWallet::SetAddressBook(const CTxDestination& address, const string& strName, const string& strPurpose)
{
    ...
    if (!fFileBacked)
        return false;
    if (!strPurpose.empty() && !CWalletDB(strWalletFile).WritePurpose(CFujicoinAddress(address).ToString(), strPurpose))
        return false;
    return CWalletDB(strWalletFile).WriteName(CFujicoinAddress(address).ToString(), strName);
}
...
bool CWallet::DelAddressBook(const CTxDestination& address)
{
    ...
    if (!fFileBacked)
        return false;
    CWalletDB(strWalletFile).ErasePurpose(CFujicoinAddress(address).ToString());
    return CWalletDB(strWalletFile).EraseName(CFujicoinAddress(address).ToString());
}
...
bool CWallet::AddDestData(const CTxDestination &dest, const std::string &key, const std::string &value)
{
    ...
    if (!fFileBacked)
        return true;
    return CWalletDB(strWalletFile).WriteDestData(CFujicoinAddress(dest).ToString(), key, value);
}
...
bool CWallet::EraseDestData(const CTxDestination &dest, const std::string &key)
{
    ...
    if (!fFileBacked)
        return true;
    return CWalletDB(strWalletFile).EraseDestData(CFujicoinAddress(dest).ToString(), key);
}
