...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"fujicoinaddress\"\n"
            "\nReturn information about the given fujicoin address.\n"
            "\nArguments:\n"
            "1. \"fujicoinaddress\"     (string, required) The fujicoin address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"fujicoinaddress\", (string) The fujicoin address validated\n"
...
UniValue createwitnessaddress(const UniValue& params, bool fHelp)
{
    ...
    CFujicoinAddress address(witscriptid);
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"fujicoinaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"fujicoinaddress\"  (string, required) The fujicoin address to use for the signature.\n"
...
{
    if (type == 2) {
        address = CFujicoinAddress(CScriptID(hash)).ToString();
    } else if (type == 1) {
        address = CFujicoinAddress(CKeyID(hash)).ToString();
    } else {
        return false;
    }
    return true;
...
bool getAddressesFromParams(const UniValue& params, std::vector<std::pair<uint160, int> > &addresses)
{
    if (params[0].isStr()) {
        ...
        for (std::vector<UniValue>::iterator it = values.begin(); it != values.end(); ++it) {
            ...
            CFujicoinAddress address(it->get_str());
            uint160 hashBytes;
            int type = 0;
            if (!address.GetIndexKey(hashBytes, type)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid address");
