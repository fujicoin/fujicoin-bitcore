...
FujicoinGUI::FujicoinGUI(const PlatformStyle *platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    clientModel(0),
    walletFrame(0),
...
void FujicoinGUI::setClientModel(ClientModel *clientModel)
{
...
void FujicoinGUI::setNumConnections(int count)
{
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to Fujicoin network", "", count));
}
    ...
    menu(0)
{
    ...
    Q_FOREACH (const FujicoinUnits::Unit unit, units)
    {
...
{
    menu = new QMenu();
    Q_FOREACH(FujicoinUnits::Unit u, FujicoinUnits::availableUnits())
    {
